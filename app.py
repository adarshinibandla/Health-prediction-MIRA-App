...
with col1:
    if st.button("➕ Predict & Create Record", width="stretch"):
        ...
        if not cleaned_name:
            st.error("Submission Denied: Full Name is required.")
        elif not cleaned_email:
            st.error("Submission Denied: Email field cannot be empty.")
        elif not is_valid_email(cleaned_email):
            st.error(f"Validation Failure: '{cleaned_email}' breaks structure standard syntax.")
        elif email_exists(cleaned_email):
            st.error(f"Database Conflict: Email '{cleaned_email}' unique-key constraint violation. Record already exists.")
        else:
            with st.spinner("Executing MIRA Classifier Inference..."):
                remarks = predict_health_risk(glucose, cholesterol)
                if is_future_date: 
                    remarks += " | System Log: Warning: Future registration timeline detected (> 2026)."
